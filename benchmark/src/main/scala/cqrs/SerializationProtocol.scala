...
object Util {
    def serializeEventLogEntry(e: EventLogEntry)(implicit f: Format[EventLogEntry]) = tobinary(e)
    def deSerializeEventLogEntry(bytes: Array[Byte])(implicit f: Format[EventLogEntry]) = frombinary[EventLogEntry](bytes)
}
